...
// * virt/creating_vms_advanced/creating_vms_advanced_web/virt-customizing-VM-creation-by-using-the-web-console.adoc
...
= Customizing a VM from an instance type by using the web console

ifdef::virt-create-vms[]
You can create a virtual machine (VM) from an instance type by using the {product-title} web console.

You can also use the web console to create a VM by copying an existing snapshot or to clone a VM.

You create a VM from a list of available bootable volumes. You can add Linux- or Windows-based volumes to the list.
endif::[]
...
Begin creating the VM by choosing between two views in the {product-title} web console:
...
====
+
** Optional: Click the star icon to designate each bootable volume you want to mark as a favorite. To see starred bootable volumes listed together either at the top or the bottom of the list, click the arrow to the left of *Volume name*.
