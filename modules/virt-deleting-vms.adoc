// Module included in the following assemblies:
//
// * virt/virtual_machines/virt-delete-vms.adoc

:_mod-docs-content-type: PROCEDURE
[id="virt-deleting-vms_{context}"]

= Deleting a virtual machine by using the CLI

You can delete a virtual machine (VM) by using the `oc` command line interface (CLI). The `oc` client enables you to perform actions on multiple VMs.

If the VM is delete protected, a message appears informing you that you must first disable the VM's delete protection before you can delete the VM.

.Prerequisites

* Identify the name of the VM that you want to delete.

.Procedure

* Delete the VM by running the following command:
+
[source,terminal]
----
$ oc delete vm <vm_name>
----
+
[NOTE]
====
This command only deletes a VM in the current project. Specify the
`-n <project_name>` option if the VM you want to delete is in
a different project or namespace.
====

.Additional resources
* link:https://openshift-enterprise/latest/virt/managing_vms/virt-delete-vms.adoc#virt-deleting-vms_virt-delete-vms[Enabling or disabling virtual machine delete protection by using the CLI]

